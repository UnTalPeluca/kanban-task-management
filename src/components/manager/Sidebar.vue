<template>
  <div v-if="managerStore.sidebar" class="z-10">
    ...
    <div class="max-w-3xs bg-white w-full fixed top-1/2 left-1/2 translate-x-[-50%] translate-y-[-50%] rounded-lg">
      <p class="text-medium-grey text-xs font-bold p-4">ALL BOARDS ({{ boardsStore.boards.length }})</p>
      <div @click="onClickBoard(index)" v-for="(board, index) in boardsStore.boards" :key="index"
        class="flex items-center gap-4 w-11/12 rounded-r-full px-4 py-3 cursor-pointer text-medium-grey font-bold   "
      ...
      <div @click="createNewBoard()"
        class=" flex items-center gap-4 w-11/12 rounded-r-full px-4 py-3 cursor-pointer font-bold fill-main-purple text-main-purple hover:bg-medium-grey/10">
        <IconBoard />
