<template>
  <div class="bg-white rounded-lg p-1" v-if="managerStore.taskView">
    <div class="p-5 pr-3 flex flex-col gap-6 max-h-[90vh] overflow-y-auto ">
      <div class="flex justify-between items-center">
        <h4 class="text-black font-bold text-lg">{{ boardsStore.getTask?.title }}</h4>
      ...
      <div>
        <p class="text-medium-grey text-xs font-bold pb-4">Subtasks ({{ subtasksCompleted }})</p>
        <div class="flex flex-col gap-2">
          <Subtask v-for="(subtask, index) in boardsStore?.getTask?.subtasks" :key="index" :subtask="subtask" />
        </div>
      </div>
      <div>
        <p class="text-medium-grey text-xs font-bold pb-4">Current Status</p>
